...
const DEFAULT_CLAUDE_LLM_PREFERENCE: ClaudeLlmPreference = {
  provider: "claude",
  model: "sonnet",
...
export function loadClaudeLlmPreference(storage?: StorageLike): ClaudeLlmPreference {
  ...
  const model = preference?.model && VALID_CLAUDE_MODELS.includes(preference.model as ClaudeModel)
    ? preference.model as ClaudeModel
...
export function saveClaudeLlmPreference(
  ...
  const model = preference.model ?? current.model;
