...
3. Identify issues:
   ...
   **Date gaps** - any interval >2 days with no session log in either `context/memory/` or `.memsearch/memory/`. List each gap with start date, end date, and length in days.
...
- Do not delete or prune anything - that is the weekly-memory-curator's job (runs right after, at 23:32)
